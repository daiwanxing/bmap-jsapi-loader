...
async function loader(params: BMapLoader.LoaderParams) {
  ...
  if (type && type !== BMAP_GL)
    return Promise.reject(`expected type value webgl but got value ${type}`);
  ...
  const awaitJobs = libs.map((data) => {
    ...
    if (!libData) {
      ...
      if (install & LOAD_STATE.LOADING)
        return new Promise((resolve) => appendUnSetteldTask(resolve, "normal"));
      return new Promise((resolve, rejected) => {
...
function addBMapGLLibrary(libs: BMapLoader.LoaderParams["library"]) {
  ...
  const awaitJobs = libs.map((data) => {
    ...
    if (!libData) {
      ...
      if (install & LOAD_STATE.LOADING)
        return new Promise((resolve) => appendUnSetteldTask(resolve, "gl"));
      return new Promise((resolve, rejected) => {
        const st = document.createElement("script");
